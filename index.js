...
/*
Prevent more than ONE active online session
per device.

MongoDB partial unique index:
only documents where status = online are unique.
*/
...
customFrom + "T00:00:00+05:30"
);

  if (!Number.isNaN(parsedFrom.getTime())) {
    from = parsedFrom.getTime();
  }
}

if (customTo) {
  const parsedTo = new Date(
    customTo + "T23:59:59.999+05:30"
  );
...
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Admin Login</title><style>
*{
...
/*
  Newly created devices and non-approved
  devices are denied.
*/
...
/*
  ...
  Duplicate-key race is handled because
  MongoDB guarantees only one online
  session per device.
*/
...
await markStaleSessionsOffline();
  ...
  /*
    A session overlaps a selected range if:
  ...
  */
  ...
  if (
    ...
    sessionMatch.startTimestamp = {
      $lte: range.to
    };

    sessionMatch.lastSeenTimestamp = {
      $gte: range.from
    };
  }

  /*
    Calculate usage only inside selected
    range.

    Example:
    Session started yesterday and continues
    today.

    Today filter counts only today's portion.
  */
  ...
  */
  ...
  /*
    ...
    Avoids loading every device into RAM.
  */
  ...
  /*
    Online devices are global and NOT tied
    to the selected usage date range.
  */
...
</form></div></div><div class="card"><form
class="filters"
id="filterForm"
><input
class="search"
...
</form></div><div class="stats"><div class="stat">
...
</h2><table><thead><tr><th>Nickname</th>
<th>Device ID</th>
<th>Permission</th>
<th>Live</th>
<th>Usage</th>
...
>
Loading...
</td></tr></tbody></table><div class="pagination"><button
id="prevPage"
...
</div><div class="modal-body"><table><thead><tr><th>Started (IST)</th>
...
tbody.innerHTML =
'<tr><td colspan="7" style="text-align:center;padding:20px;">No devices found.</td></tr>';
...
actionButtons +=
'<button class="btn history-btn" type="button" data-history="' +
deviceId +
'" data-nickname="' +
nickname +
'">History</button>';
...
'<input type="hidden" name="_csrf" value="' + csrfToken + '">' +
'<input type="hidden" name="deviceId" value="' + deviceId + '">' +
'<button class="btn block" type="submit">Block</button>' +
'</form>';
...
actionButtons +=
'<form method="POST" action="/action/pending">' +
'<input type="hidden" name="_csrf" value="' + csrfToken + '">' +
'<input type="hidden" name="deviceId" value="' + deviceId + '">' +
'<button class="btn pending-btn" type="submit">Pending</button>' +
'</form>';

}

actionButtons +=
'<form method="POST" action="/action/delete" onsubmit="return confirm(\\'Delete this device permanently?\\');">' +
'<input type="hidden" name="_csrf" value="' + csrfToken + '">' +
'<input type="hidden" name="deviceId" value="' + deviceId + '">' +
...
'<form method="POST" action="/action/nickname" style="display:flex;gap:5px;">' +
...
'<input class="nickname-input" name="nickname" value="' +
nickname +
'" placeholder="Nickname" maxlength="50">' +

'<button class="btn refresh-btn" type="submit">Save</button>' +
...
tableBody.innerHTML =
'<tr><td colspan="4" style="text-align:center;">Fetching records...</td></tr>';
...
tableBody.innerHTML =
'<tr><td colspan="4" style="text-align:center;">No sessions recorded yet.</td></tr>';
...
tableBody.innerHTML =
'<tr><td colspan="4" style="text-align:center;color:#f87171;">Failed to load history.</td></tr>';
...
/*
  ...
  Buttons and hidden inputs do not.
*/
...
/*
  Pause auto-refresh when browser tab
  is hidden.
*/
...
app.listen(
  ...
  () => {
    console.log(
      "Ultimate Admin Dashboard V5 running on port " +
      PORT
    );
  }
);
