...
/* =========================================================
   V6.1 DEVICE & APK MANAGEMENT CONSOLE
...
const apiLimiter = rateLimit({
  windowMs: 60 * 1000,
  max: 100, // Limit for public APK JSON fetch
  standardHeaders: true,
  legacyHeaders: false
});
...
const SessionSchema = new mongoose.Schema({
  ...
  endReason: { type: String, enum: ["stop", "timeout", "blocked", "pending", "deleted", "history_cleared", null], default: null }
}, { versionKey: false });

SessionSchema.index({ deviceId: 1, startTimestamp: -1 });
SessionSchema.index({ status: 1, lastSeenTimestamp: 1 });
SessionSchema.index({ deviceId: 1 }, { unique: true, partialFilterExpression: { status: "online" }, name: "unique_online_session_per_device" });
...
const Apk = mongoose.model("Apk", ApkSchema); // New APK Model
...
/* =========================================================
   SESSION MANAGEMENT
========================================================= */
async function closeOnlineSession(deviceId, reason, timestamp) {
  const now = Number(timestamp) || Date.now();
  const nowDate = new Date(now);
  const sessionDoc = await UsageSession.findOneAndUpdate(
    { deviceId, status: "online" },
    { $set: { status: "offline", endReason: reason, endTime: nowDate, endTimestamp: now, lastSeenTime: nowDate, lastSeenTimestamp: now } },
    ...
    const operations = staleSessions.map((item) => {
      ...
      return {
        updateOne: {
          ...
          update: { $set: { status: "offline", endReason: "timeout", endTime: new Date(endTimestamp), endTimestamp, durationMs: duration } }
        }
      };
    });
...
body{margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;background:#f3f4f6;padding:20px;}
.card{width:100%;max-width:420px;background:white;border:1px solid #e5e7eb;border-radius:12px;padding:32px;box-shadow:0 10px 30px rgba(0,0,0,.08);}
h1{margin:0 0 8px;font-size:24px;}
p{color:#6b7280;font-size:14px;margin-bottom:22px;}
label{display:block;font-size:13px;font-weight:600;margin:14px 0 6px;}
input{width:100%;padding:12px;border:1px solid #d1d5db;border-radius:8px;font-size:14px;}
button{width:100%;margin-top:20px;padding:12px;border:0;border-radius:8px;background:#111827;color:white;font-weight:600;cursor:pointer;}
.error{background:#fef2f2;color:#b91c1c;border:1px solid #fecaca;padding:10px;border-radius:7px;font-size:13px;margin-bottom:15px;}
</style>
</head>
<body>
<div class="card">
  ...
  try {
    ...
    req.session.regenerate((err) => {
      ...
      req.session.save((saveErr) => {
        if (saveErr) { return res.redirect("/login?error=1"); }
        return res.redirect("/");
      });
    });
  } catch (err) {
...
app.post("/logout", requireLogin, csrfProtection, (req, res) => {
  req.session.destroy(() => { res.clearCookie("admin.sid"); res.redirect("/login"); });
});

app.get("/health", (req, res) => {
  const connected = mongoose.connection.readyState === 1;
  res.status(connected ? 200 : 503).json({ server: "online", database: connected ? "connected" : "disconnected", timestamp: new Date().toISOString() });
});
  ...
  try {
    ...
    if (!device) {
      try {
        ...
        if (err && err.code === 11000) { device = await Device.findOne({ deviceId }); } else { throw err; }
      }
    }

    if (!device || device.status !== "approved") {
      await closeOnlineSession(deviceId, device && device.status === "blocked" ? "blocked" : "pending", Date.now());
    ...
    if (action === "stop") {
      const stopped = await closeOnlineSession(deviceId, "stop", now);
      return res.json({ status: "ALLOWED", action: stopped ? "STOPPED" : "NO_ACTIVE_SESSION" });
    }

    let activeSession = await UsageSession.findOne({ deviceId, status: "online" });
    if (activeSession) {
    ...
    try {
      activeSession = await UsageSession.create({ deviceId, startTime: nowDate, lastSeenTime: nowDate, startTimestamp: now, lastSeenTimestamp: now, status: "online" });
    } catch (err) {
      if (err && err.code === 11000) {
        activeSession = await UsageSession.findOneAndUpdate({ deviceId, status: "online" }, { $set: { lastSeenTime: nowDate, lastSeenTimestamp: now } }, { new: true });
      } else { throw err; }
    }
...
app.post("/action/:type", requireLogin, csrfProtection, async (req, res) => {
  ...
  if (!deviceId && type !== "clear-all-history") { return res.redirect(req.get("referer") || "/"); }

  try {
    if (type === "nickname") {
      const nickname = safeString(req.body.nickname, MAX_NICKNAME_LENGTH);
      await Device.updateOne({ deviceId }, { $set: { nickname } });
    }
    if (type === "approve") { await Device.updateOne({ deviceId }, { $set: { status: "approved" } }); }
    if (type === "pending") {
      await Device.updateOne({ deviceId }, { $set: { status: "pending" } });
      await closeOnlineSession(deviceId, "pending", Date.now());
    }
    if (type === "block") {
      await Device.updateOne({ deviceId }, { $set: { status: "blocked" } });
      await closeOnlineSession(deviceId, "blocked", Date.now());
    }
    if (type === "clear-history") { await UsageSession.deleteMany({ deviceId }); }
    if (type === "delete") {
      await Promise.all([Device.deleteOne({ deviceId }), UsageSession.deleteMany({ deviceId })]);
    }
    if (type === "clear-all-history") { await UsageSession.deleteMany({}); }
  } catch (err) { console.error("Admin action error:", err.message); }
...
app.post("/action/apk/add", requireLogin, csrfProtection, async (req, res) => {
  try {
    await Apk.create({
      appName: safeString(req.body.appName, 100),
      description: safeString(req.body.description, 500),
      versionName: safeString(req.body.versionName, 50),
      versionCode: parseInt(req.body.versionCode, 10) || 1,
      apkUrl: safeString(req.body.apkUrl, 500)
    });
  } catch(err) { console.error("Add APK error:", err.message); }
  res.redirect("/apks");
});

app.post("/action/apk/delete", requireLogin, csrfProtection, async (req, res) => {
  try {
    await Apk.findByIdAndDelete(req.body.id);
  } catch(err) { console.error("Delete APK error:", err.message); }
  res.redirect("/apks");
});
...
app.get("/api/sessions/:deviceId", requireApiLogin, async (req, res) => {
  try {
    ...
    const sessions = await UsageSession.find({ deviceId }).sort({ startTimestamp: -1 }).limit(200).lean();
    const now = Date.now();
    const formatted = sessions.map((item) => {
      ...
      return {
        ...
        endTime: item.endTime ? safeDate(item.endTime) : item.status === "online" ? "Currently active" : safeDate(item.lastSeenTime),
        duration: formatDuration(duration),
        durationMs: duration,
        status: item.status,
        endReason: item.endReason || (item.status === "online" ? "active" : "unknown")
      };
    });
...
app.get("/api/dashboard", requireApiLogin, async (req, res) => {
  try {
    ...
    const usagePipeline = [
      { $match: sessionMatch },
      { $project: { deviceId: 1, durationStart: durationStartExpression, durationEnd: durationEndExpression } },
    ...
    const deviceStatusStats = results[0];
    const totalSearchDevices = results[1];
    let devices = results[2];
    const usageStats = results[3];
    const chartStats = results[4];
    const onlineSessions = results[5];
    ...
    usageStats.forEach((item) => {
      ...
      usageMap[String(item._id)] = { totalUsage: usage, sessionCount: Number(item.sessionCount || 0) };
      totalUsage += usage;
    });
    ...
    const deviceData = devices.map((device) => {
      const stat = usageMap[String(device.deviceId)] || { totalUsage: 0, sessionCount: 0 };
      return {
        deviceId: device.deviceId, nickname: device.nickname || "", status: device.status,
        registeredAt: safeDate(device.registeredAt), usage: formatDuration(stat.totalUsage),
        sessions: stat.sessionCount, online: onlineSet.has(String(device.deviceId))
      };
    });
    ...
    return res.json({
      ...
      chart: { labels: chartStats.map((item) => item._id), data: chartStats.map((item) => Math.round(Number(item.usage || 0) / 60000)) }
    });
  } catch (err) {
...
const UI_STYLES = `
<style>
:root{--bg:#f5f6f8;--border:#e5e7eb;--text:#111827;--blue:#2563eb;}
*{box-sizing:border-box;font-family:Inter,system-ui,sans-serif;}
body{margin:0;background:var(--bg);color:var(--text);}
.topbar{height:64px;background:#111827;color:white;display:flex;align-items:center;justify-content:space-between;padding:0 24px;}
.brand{font-size:17px;font-weight:700;}
.brand span{color:#9ca3af;font-weight:400;margin-left:8px;font-size:13px;}
...
.card{background:white;border:1px solid var(--border);border-radius:10px;margin-bottom:18px;}
.card-header{padding:16px 18px;border-bottom:1px solid var(--border);font-weight:650;font-size:14px;}
...
input,select,textarea{padding:9px 11px;border:1px solid #d1d5db;border-radius:7px;font-size:13px;background:white;}
...
.btn-dark{background:#111827;} .btn-blue{background:#2563eb;} .btn-green{background:#15803d;} .btn-orange{background:#c2410c;} .btn-yellow{background:#a16207;} .btn-red{background:#b91c1c;} .btn-purple{background:#6d28d9;} .btn-gray{background:#e5e7eb;color:#111827;}
.btn:hover{opacity:.9;}
.top-actions{display:flex;gap:8px;align-items:center;}
.table-wrap{overflow-x:auto;}
table{width:100%;border-collapse:collapse;min-width:1100px;}
th{background:#f9fafb;color:#6b7280;font-size:11px;text-transform:uppercase;}
th,td{padding:13px;border-bottom:1px solid var(--border);text-align:left;font-size:13px;}
code{font-size:11px;background:#f3f4f6;padding:4px 6px;border-radius:4px;word-break:break-all;}
.badge{display:inline-block;padding:5px 8px;border-radius:20px;font-size:10px;font-weight:700;}
...
.action-cell{display:flex;gap:5px;flex-wrap:wrap;min-width:430px;}
.inline-form{margin:0;display:inline-flex;gap:5px;}
.nickname-input{width:125px;padding:7px;font-size:12px;}
.modal-overlay{display:none;position:fixed;inset:0;background:rgba(17,24,39,.55);z-index:9999;padding:20px;align-items:center;justify-content:center;}
.modal{width:100%;max-width:900px;background:white;border-radius:12px;overflow:hidden;box-shadow:0 25px 60px rgba(0,0,0,.25);}
...
@media(max-width:700px){.topbar{padding:0 14px;}.container{padding:14px;}.brand span{display:none;}.search{width:100%;min-width:0;}}
</style>
`;

const TOPBAR_HTML = (csrfToken) => `
<div class="topbar">
<div class="brand">Admin Console <span>V6.1 Control Panel</span></div>
...
app.get("/apks", requireLogin, csrfProtection, async (req, res) => {
  ...
  let apkRows = apks.map(apk => `
    <tr>
      <td><strong>${escapeHtml(apk.appName)}</strong><br><span class="status-line">${escapeHtml(apk.description)}</span></td>
      <td><span class="badge online">${escapeHtml(apk.versionName)}</span> (Code: ${apk.versionCode})</td>
      <td><a href="${escapeHtml(apk.apkUrl)}" target="_blank" style="color:var(--blue);font-size:12px;">Link</a></td>
      ...
      <td>
        <form class="inline-form" method="POST" action="/action/apk/delete" onsubmit="return confirm('Delete this APK from the store?')">
          <input type="hidden" name="_csrf" value="${escapeHtml(res.locals.csrfToken)}">
          <input type="hidden" name="id" value="${apk._id}">
  ...
  if(!apkRows) apkRows = '<tr><td colspan="5" style="text-align:center;padding:25px">No APKs uploaded yet.</td></tr>';

  res.send(`
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>APK Manager</title>
${UI_STYLES}
</head>
<body>
${TOPBAR_HTML(res.locals.csrfToken)}
<div class="container">
<div class="page-title"><h1>APK App Store Manager</h1><p class="status-line">Manage your modded apps and updates sent to clients.</p></div>

<div class="card">
<div class="card-header">Upload New Update / App</div>
<div class="card-body">
<form method="POST" action="/action/apk/add" style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
  <input type="hidden" name="_csrf" value="${escapeHtml(res.locals.csrfToken)}">
  <div class="form-group"><label>App Name</label><input type="text" name="appName" required placeholder="e.g. Netflix Mod"></div>
  <div class="form-group"><label>Direct APK Link (URL)</label><input type="url" name="apkUrl" required placeholder="https://github.com/.../app.apk"></div>
  <div class="form-group"><label>Version Name</label><input type="text" name="versionName" required placeholder="e.g. v2.0"></div>
  <div class="form-group"><label>Version Code</label><input type="number" name="versionCode" required placeholder="e.g. 2"></div>
  <div class="form-group" style="grid-column: 1 / -1;"><label>Description (Features / Patch Notes)</label><textarea name="description" rows="2" placeholder="What's new in this update?"></textarea></div>
  <div style="grid-column: 1 / -1;"><button type="submit" class="btn btn-green">Publish Update</button></div>
</form>
</div>
</div>

<div class="card">
<div class="card-header">Published Apps (Live on Android App)</div>
<div class="table-wrap">
<table style="min-width: 800px;">
<thead><tr><th>App Info</th><th>Version</th><th>Download Link</th><th>Uploaded On</th><th>Action</th></tr></thead>
<tbody>${apkRows}</tbody>
</table>
</div>
</div>
...
<html>
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Device Console</title>
...
<input id="search" class="search" placeholder="Search device ID, nickname or status">
<select id="filter"><option value="all">All Time</option><option value="today">Today (IST)</option><option value="7d">Last 7 Days</option><option value="30d">Last 30 Days</option><option value="custom">Custom Range</option></select>
<input type="date" id="from"><input type="date" id="to">
<button class="btn btn-blue" type="submit">Apply Filter</button>
<button type="button" class="btn btn-gray" onclick="manualRefresh()">Refresh</button>
</form>
<div style="margin-top:12px;">
<form method="POST" action="/action/clear-all-history" onsubmit="return confirm('WARNING: Permanently delete ALL session history? Devices stay safe.')">
<input type="hidden" name="_csrf" value="${escapeHtml(res.locals.csrfToken)}">
<button type="submit" class="btn btn-red">🗑 Clear All History</button>
</form>
</div>
</div></div>

<div class="stats">
<div class="stat"><div class="stat-label">TOTAL DEVICES</div><div class="stat-value" id="totalDevices">-</div></div>
<div class="stat"><div class="stat-label">APPROVED</div><div class="stat-value" id="approved">-</div></div>
<div class="stat"><div class="stat-label">PENDING</div><div class="stat-value" id="pending">-</div></div>
<div class="stat"><div class="stat-label">BLOCKED</div><div class="stat-value" id="blocked">-</div></div>
<div class="stat"><div class="stat-label">ONLINE NOW</div><div class="stat-value" id="online">-</div></div>
<div class="stat"><div class="stat-label">TOTAL USAGE</div><div class="stat-value" id="totalUsage">-</div></div>
</div>
...
<div class="card"><div class="card-header">Device Permission Manager</div><div class="table-wrap">
<table><thead><tr><th>Nickname</th><th>Device ID</th><th>Permission</th><th>Live Status</th><th>Usage</th><th>Registered</th><th>Actions</th></tr></thead>
<tbody id="deviceTable"><tr><td colspan="7" style="text-align:center;padding:25px">Loading devices...</td></tr></tbody></table>
</div>
<div class="pagination"><button id="prevPage">Previous</button><span id="pageInfo">Page -</span><button id="nextPage">Next</button></div>
</div>

</div>

<!-- HISTORY MODAL -->
<div class="modal-overlay" id="historyModal"><div class="modal"><div class="modal-header"><div><div class="modal-title" id="modalTitle">Session History</div></div><button class="modal-close" onclick="closeModal()">×</button></div>
<div class="modal-body"><div class="table-wrap"><table><thead><tr><th>Started</th><th>Last Seen</th><th>Ended</th><th>Duration</th><th>Status</th><th>End Reason</th></tr></thead><tbody id="historyTableBody"><tr><td colspan="6" style="text-align:center">Loading...</td></tr></tbody></table></div></div></div></div>

<script>
const csrfToken = "${escapeHtml(res.locals.csrfToken)}";
const REFRESH_SECONDS = ${DASHBOARD_REFRESH_SECONDS};
let currentPage = 1, chartInstance = null, refreshTimer = null, refreshInProgress = false;
...
async function refreshDashboard(){
  ...
  refreshInProgress = true; clearTimeout(refreshTimer);
  try{
    refreshStatus.textContent = "Refreshing data...";
    const params = new URLSearchParams();
    params.set("search", document.getElementById("search").value); params.set("filter", document.getElementById("filter").value);
    params.set("from", document.getElementById("from").value); params.set("to", document.getElementById("to").value); params.set("page", currentPage);
    ...
    updateStats(data.stats); updateChart(data.chart); updateTable(data.devices); updatePagination(data.pagination);
    refreshStatus.textContent = "Last refreshed: " + new Date().toLocaleTimeString("en-IN");
  }catch(error){
    console.error(error); refreshStatus.textContent = "Unable to refresh dashboard.";
  }finally{
...
function updateStats(stats){
  document.getElementById("totalDevices").textContent = stats.totalDevices; document.getElementById("approved").textContent = stats.approved;
  document.getElementById("pending").textContent = stats.pending; document.getElementById("blocked").textContent = stats.blocked;
  document.getElementById("online").textContent = stats.online; document.getElementById("totalUsage").textContent = stats.totalUsage;
}

function updateChart(chart){
  const canvas = document.getElementById("usageChart");
  if(chartInstance) chartInstance.destroy();
  chartInstance = new Chart(canvas, { type:"line", data:{ labels: chart.labels, datasets:[{ label: "Usage (minutes)", data: chart.data, borderColor: "#2563eb", backgroundColor: "rgba(37,99,235,.08)", borderWidth:2, fill:true, tension:.25 }] }, options:{ responsive:true, maintainAspectRatio:false, scales:{ y:{ beginAtZero:true } } } });
}
...
function updateTable(devices){
  ...
  tbody.innerHTML = devices.map(function(device){
    const deviceId = escapeHTML(device.deviceId), nickname = escapeHTML(device.nickname), status = escapeHTML(device.status);
    const liveClass = device.online ? "online" : "offline", liveText = device.online ? "ONLINE" : "OFFLINE";
    let actions = '<button type="button" class="btn btn-purple" data-history="'+deviceId+'" data-nickname="'+nickname+'">History</button>';
    actions += '<form class="inline-form" method="POST" action="/action/clear-history" onsubmit="return confirm(\\'Delete ONLY this device session history?\\')"><input type="hidden" name="_csrf" value="'+csrfToken+'"><input type="hidden" name="deviceId" value="'+deviceId+'"><button class="btn btn-red" type="submit">Clear History</button></form>';
    if(device.status !== "approved") actions += '<form class="inline-form" method="POST" action="/action/approve"><input type="hidden" name="_csrf" value="'+csrfToken+'"><input type="hidden" name="deviceId" value="'+deviceId+'"><button class="btn btn-green" type="submit">Approve</button></form>';
    if(device.status !== "blocked") actions += '<form class="inline-form" method="POST" action="/action/block"><input type="hidden" name="_csrf" value="'+csrfToken+'"><input type="hidden" name="deviceId" value="'+deviceId+'"><button class="btn btn-orange" type="submit">Block</button></form>';
    if(device.status !== "pending") actions += '<form class="inline-form" method="POST" action="/action/pending"><input type="hidden" name="_csrf" value="'+csrfToken+'"><input type="hidden" name="deviceId" value="'+deviceId+'"><button class="btn btn-yellow" type="submit">Pending</button></form>';
    actions += '<form class="inline-form" method="POST" action="/action/delete" onsubmit="return confirm(\\'Delete DEVICE and ALL history permanently?\\')"><input type="hidden" name="_csrf" value="'+csrfToken+'"><input type="hidden" name="deviceId" value="'+deviceId+'"><button class="btn btn-red" type="submit">Delete Device</button></form>';
    
    return '<tr><td><form class="inline-form" method="POST" action="/action/nickname"><input type="hidden" name="_csrf" value="'+csrfToken+'"><input type="hidden" name="deviceId" value="'+deviceId+'"><input class="nickname-input" name="nickname" maxlength="50" placeholder="Nickname" value="'+nickname+'"><button class="btn btn-blue" type="submit">Save</button></form></td><td><code>'+deviceId+'</code></td><td><span class="badge '+status+'">'+status.toUpperCase()+'</span></td><td><span class="badge '+liveClass+'">'+liveText+'</span></td><td><strong>'+escapeHTML(device.usage)+'</strong><br><span style="font-size:11px;color:#6b7280">'+Number(device.sessions)+' sessions</span></td><td>'+escapeHTML(device.registeredAt)+'</td><td class="action-cell">'+actions+'</td></tr>';
  }).join("");
  document.querySelectorAll("[data-history]").forEach(function(button){ button.addEventListener("click", function(){ openHistory(button.getAttribute("data-history"), button.getAttribute("data-nickname")); }); });
}

async function openHistory(deviceId, nickname){
  const modal = document.getElementById("historyModal"), title = document.getElementById("modalTitle"), body = document.getElementById("historyTableBody");
  title.textContent = "Session History — " + (nickname || deviceId);
  body.innerHTML = '<tr><td colspan="6" style="text-align:center">Loading session history...</td></tr>';
  ...
  }catch(error){ console.error(error); body.innerHTML = '<tr><td colspan="6" style="text-align:center;color:#b91c1c">Failed to load session history.</td></tr>'; }
}
...
async function shutdown(signal) {
  ...
  try {
    ...
    process.exit(0);
  } catch (err) { console.error("Shutdown error:", err.message); process.exit(1); }
}
...
async function startServer() {
  try {
    ...
    try { await Device.init(); } catch (err) { console.warn("Device index warning:", err.message); }
    try { await UsageSession.init(); } catch (err) { console.warn("Session index warning:", err.message); }
    try { await Apk.init(); } catch (err) { console.warn("Apk index warning:", err.message); }
    
    console.log("MongoDB indexes ready");
    server = app.listen(PORT, () => {
      console.log("V6.1 Device Console running on port " + PORT);
    });
  } catch (err) {
