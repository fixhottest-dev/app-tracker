...
/* =========================================================
   V6.3 MULTI-APP DEVICE & APK MANAGEMENT CONSOLE
========================================================= */
...
/* =========================================================
   DATABASE SCHEMAS (UPDATED FOR MULTI-APP)
========================================================= */
...
// Device + App combination must be unique
...
// Only one online session per device per app
SessionSchema.index(
  { deviceId: 1, appId: 1 },
...
async function markStaleSessionsOffline() {
  ...
  try {
    ...
    const staleSessions = await UsageSession
      .find({ status: "online", lastSeenTimestamp: { $lt: cutoff } })
      .select({ _id: 1, startTimestamp: 1, lastSeenTimestamp: 1 })
      .lean();
  ...
  } catch (err) { console.error("Stale cleanup error:", err.message); } finally { cleanupRunning = false; }
}
...
/* =========================================================
   TRACKING API (V6.3 MULTI-APP ENABLED)
========================================================= */
  ...
  try {
    ...
    try {
      activeSession = await UsageSession.create({
        deviceId, appId, startTime: nowDate, lastSeenTime: nowDate, startTimestamp: now, lastSeenTimestamp: now, status: "online"
      });
    } catch (err) {
...
app.get("/login", csrfProtection, (req, res) => {
  ...
  res.send(`
    ...
    <style>*{box-sizing:border-box;font-family:Inter,system-ui,sans-serif;}body{margin:0;min-height:100vh;display:flex;align-items:center;justify-content:center;background:#f3f4f6;padding:20px;}.card{width:100%;max-width:420px;background:white;border:1px solid #e5e7eb;border-radius:12px;padding:32px;box-shadow:0 10px 30px rgba(0,0,0,.08);}h1{margin:0 0 8px;font-size:24px;}p{color:#6b7280;font-size:14px;margin-bottom:22px;}label{display:block;font-size:13px;font-weight:600;margin:14px 0 6px;}input{width:100%;padding:12px;border:1px solid #d1d5db;border-radius:8px;font-size:14px;}button{width:100%;margin-top:20px;padding:12px;border:0;border-radius:8px;background:#111827;color:white;font-weight:600;cursor:pointer;}.error{background:#fef2f2;color:#b91c1c;border:1px solid #fecaca;padding:10px;border-radius:7px;font-size:13px;margin-bottom:15px;}</style></head>
    <body><div class="card"><h1>Administrator Login</h1><p>Sign in to access the management console.</p>
    ${req.query.error ? '<div class="error">Invalid username or password.</div>' : ""}
    <form method="POST" action="/login"><input type="hidden" name="_csrf" value="${escapeHtml(res.locals.csrfToken)}"><label>Username</label><input type="text" name="username" autocomplete="username" required><label>Password</label><input type="password" name="password" autocomplete="current-password" required><button type="submit">Sign In</button></form></div></body></html>
  `);
});
...
app.post("/login", loginLimiter, csrfProtection, async (req, res) => {
  ...
  } catch (err) { return res.redirect("/login?error=1"); }
});
...
/* =========================================================
   ADMIN DEVICE ACTIONS (UPDATED FOR MULTI-APP)
========================================================= */
app.post("/action/:type", requireLogin, csrfProtection, async (req, res) => {
  ...
  try {
    if (type === "nickname") {
      ...
      await Device.updateOne({ deviceId, appId }, { $set: { nickname } });
    }
...
app.post("/action/apk/add", requireLogin, csrfProtection, async (req, res) => {
  try {
    ...
    if (!appName || !packageName || !versionName || !apkUrl) return res.redirect("/apks?error=missing");
    await Apk.create({ appName, description: safeString(req.body.description, 500), versionName, versionCode: parseInt(req.body.versionCode, 10) || 1, packageName, apkUrl });
...
app.post("/action/apk/delete", requireLogin, csrfProtection, async (req, res) => {
  try { const id = safeString(req.body.id, 100); if (mongoose.Types.ObjectId.isValid(id)) await Apk.findByIdAndDelete(id); }
  catch (err) { console.error("Delete APK error:", err.message); }
  return res.redirect("/apks");
});

/* =========================================================
   SESSION HISTORY API (UPDATED)
========================================================= */
app.get("/api/sessions/:deviceId", requireApiLogin, async (req, res) => {
  try {
    ...
    if (!deviceId || !appId) return res.status(400).json({ success: false, error: "DEVICE_ID_AND_APP_ID_REQUIRED" });
    ...
    const formatted = sessions.map((item) => {
      ...
      return {
        startTime: safeDate(item.startTime), lastSeenTime: safeDate(item.lastSeenTime),
        endTime: item.endTime ? safeDate(item.endTime) : item.status === "online" ? "Currently active" : safeDate(item.lastSeenTime),
        duration: formatDuration(duration), durationMs: duration, status: item.status,
        endReason: item.endReason || (item.status === "online" ? "active" : "unknown")
      };
    });
    return res.json({ success: true, sessions: formatted });
  } catch (err) { return res.status(500).json({ success: false, error: "FETCH_FAILED" }); }
});

/* =========================================================
   DASHBOARD API (UPDATED FOR MULTI-APP)
========================================================= */
app.get("/api/dashboard", requireApiLogin, async (req, res) => {
  try {
    ...
    const appFilter = String(req.query.appFilter || "all");
    ...
    const sessionMatch = range.from !== null ? {
      ...
      $or: [ { endTimestamp: { $gte: range.from } }, { endTimestamp: null, lastSeenTimestamp: { $gte: range.from } }, { status: "online" } ]
    } : {};
    ...
    if (appFilter && appFilter !== "all") onlineMatch.appId = appFilter;
    ...
    const results = await Promise.all([
      ...
      Device.distinct("appId") // Fetches all unique appIds for the dropdown
    ]);
    ...
    return res.json({
      ...
      chart: { labels: chartStats.map(i => i._id), data: chartStats.map(i => Math.round(Number(i.usage || 0) / 60000)) }
    });
  } catch (err) {
...
.badge-app{background:#e0e7ff;color:#4338ca;font-size:10px;margin-top:4px;}
.approved,.online{background:#dcfce7;color:#166534;}
.pending{background:#fef3c7;color:#92400e;}
.blocked,.offline{background:#fee2e2;color:#991b1b;}
...
.pagination{display:flex;gap:12px;align-items:center;padding:15px;justify-content:center;}
.pagination button{padding:8px 14px;border:1px solid #ddd;border-radius:6px;cursor:pointer;}
...
<div class="topbar">
  <div class="brand">Admin Console<span>V6.3 Multi-App Server</span></div>
...
app.get("/apks", requireLogin, csrfProtection, async (req, res) => {
  try {
    ...
    return res.send(`
      <!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>APK Manager</title>${UI_STYLES}</head>
      <body>${TOPBAR_HTML(res.locals.csrfToken)}<div class="container"><div class="page-title"><h1>APK App Store Manager</h1><p class="status-line">Manage Android applications and updates.</p></div>
      <div class="card"><div class="card-header">Publish New APK</div><div class="card-body">
      <form method="POST" action="/action/apk/add" style="display:grid;grid-template-columns:1fr 1fr;gap:15px;"><input type="hidden" name="_csrf" value="${escapeHtml(res.locals.csrfToken)}">
      <div class="form-group"><label>App Name</label><input type="text" name="appName" required placeholder="Example: My App"></div>
      <div class="form-group"><label>Package Name</label><input type="text" name="packageName" required placeholder="com.example.app"></div>
      <div class="form-group"><label>Version Name</label><input type="text" name="versionName" required placeholder="Example: 2.0"></div>
      <div class="form-group"><label>Version Code</label><input type="number" name="versionCode" required placeholder="Example: 20"></div>
      ...
      <div class="form-group" style="grid-column:1 / -1;"><label>Description / Changelog</label><textarea name="description" rows="3" placeholder="What's new in this version?"></textarea></div>
      <div style="grid-column:1 / -1;"><button type="submit" class="btn btn-green">Publish APK</button></div></form></div></div>
      <div class="card"><div class="card-header">Published Apps</div><div class="table-wrap"><table style="min-width:900px;"><thead><tr><th>App</th><th>Version</th><th>Package Name</th><th>APK</th><th>Published</th><th>Action</th></tr></thead><tbody>${apkRows}</tbody></table></div></div></div></body></html>
    `);
  } catch (err) { return res.status(500).send("Failed to load APK Manager."); }
});

/* =========================================================
   MAIN DASHBOARD (UPDATED FOR APP ID)
========================================================= */
app.get("/", requireLogin, csrfProtection, (req, res) => {
  res.send(`
    ...
    <head>
      ...
      <title>Device Console V6.3</title>
    ...
    <body>
      ...
      <div class="container">
        <div class="page-title">
          ...
          <p id="refreshStatus" class="status-line">Loading dashboard...</p>
        </div>
        
        <div class="card">
          <div class="card-body">
            <form id="filterForm" class="filters">
              <input id="search" class="search" placeholder="Search device ID or nickname">
              <select id="appFilter">
            ...
            <div style="margin-top:12px;">
              <form method="POST" action="/action/clear-all-history" onsubmit="return confirm('WARNING: Permanently delete ALL session history for ALL apps?')">
                <input type="hidden" name="_csrf" value="${escapeHtml(res.locals.csrfToken)}">
        ...
        <div class="card">
          <div class="card-header">Device Permissions</div>
          <div class="table-wrap">
            <table>
              <thead>
                <tr><th>Nickname</th><th>Device ID & App</th><th>Permission</th><th>Live Status</th><th>Usage</th><th>Registered</th><th>Actions</th></tr>
              </thead>
              <tbody id="deviceTable">
                <tr><td colspan="7" style="text-align:center;padding:25px;">Loading devices...</td></tr>
              </tbody>
            </table>
          </div>
      ...
      <div class="modal-overlay" id="historyModal">
        <div class="modal">
          <div class="modal-header">
            <div id="modalTitle">Session History</div>
          ...
          <div class="modal-body">
            <div class="table-wrap">
              <table>
                <thead>
                  <tr><th>Started</th><th>Last Seen</th><th>Ended</th><th>Duration</th><th>Status</th><th>Reason</th></tr>
                </thead>
      ...
      <script>
        ...
        async function refreshDashboard(){
          ...
          try{
            ...
            refreshStatus.textContent = "Unable to refresh dashboard.";
          } finally {
        ...
        function updateAppDropdown(apps) {
          ...
          apps.forEach(app => {
            if(app && app.trim() !== ""){
              ...
              if(app === currentValue) option.selected = true;
        ...
        function updateTable(devices){
          ...
          tbody.innerHTML = devices.map(function(device){
            ...
            let actions = '<button type="button" class="btn btn-purple" data-history="' + deviceId + '" data-app="' + appId + '" data-nickname="' + nickname + '">History</button>';
            actions += '<form class="inline-form" method="POST" action="/action/clear-history"><input type="hidden" name="_csrf" value="' + csrfToken + '"><input type="hidden" name="deviceId" value="' + deviceId + '"><input type="hidden" name="appId" value="' + appId + '"><button class="btn btn-red" type="submit">Clear History</button></form>';
            
            if(device.status !== "approved"){ actions += '<form class="inline-form" method="POST" action="/action/approve"><input type="hidden" name="_csrf" value="' + csrfToken + '"><input type="hidden" name="deviceId" value="' + deviceId + '"><input type="hidden" name="appId" value="' + appId + '"><button class="btn btn-green" type="submit">Approve</button></form>'; }
            if(device.status !== "blocked"){ actions += '<form class="inline-form" method="POST" action="/action/block"><input type="hidden" name="_csrf" value="' + csrfToken + '"><input type="hidden" name="deviceId" value="' + deviceId + '"><input type="hidden" name="appId" value="' + appId + '"><button class="btn btn-orange" type="submit">Block</button></form>'; }
            if(device.status !== "pending"){ actions += '<form class="inline-form" method="POST" action="/action/pending"><input type="hidden" name="_csrf" value="' + csrfToken + '"><input type="hidden" name="deviceId" value="' + deviceId + '"><input type="hidden" name="appId" value="' + appId + '"><button class="btn btn-yellow" type="submit">Pending</button></form>'; }
            actions += '<form class="inline-form" method="POST" action="/action/delete"><input type="hidden" name="_csrf" value="' + csrfToken + '"><input type="hidden" name="deviceId" value="' + deviceId + '"><input type="hidden" name="appId" value="' + appId + '"><button class="btn btn-red" type="submit">Delete Device</button></form>';

            return '<tr>' +
              '<td><form class="inline-form" method="POST" action="/action/nickname"><input type="hidden" name="_csrf" value="' + csrfToken + '"><input type="hidden" name="deviceId" value="' + deviceId + '"><input type="hidden" name="appId" value="' + appId + '"><input class="nickname-input" name="nickname" maxlength="50" placeholder="Nickname" value="' + nickname + '"><button class="btn btn-blue" type="submit">Save</button></form></td>' +
              '<td><code>' + deviceId + '</code><br><span class="badge badge-app">' + appId + '</span></td>' +
              '<td><span class="badge ' + status + '">' + status.toUpperCase() + '</span></td>' +
              '<td><span class="badge ' + liveClass + '">' + liveText + '</span></td>' +
              '<td><strong>' + escapeHTML(device.usage) + '</strong><br><span style="font-size:11px;color:#6b7280">' + Number(device.sessions) + ' sessions</span></td>' +
          ...
          document.querySelectorAll("[data-history]").forEach(function(button){
            button.addEventListener("click", function(){ openHistory(button.getAttribute("data-history"), button.getAttribute("data-app"), button.getAttribute("data-nickname")); });
          });
        }

        async function openHistory(deviceId, appId, nickname){
          const modal = document.getElementById("historyModal");
          const title = document.getElementById("modalTitle");
          const body = document.getElementById("historyTableBody");
          
          title.textContent = "History — " + (nickname || deviceId) + " (" + appId + ")";
